...
def get_average_precision(targets, outputs):
    targets = targets.reshape(targets.shape[0], -1)
    outputs = outputs[:, :, :, :, 1]
...
def compute_iou_for_binary_segmentation(y_argmax, target):
    In = numpy.logical_and(y_argmax == 1, target == 1).sum()
...
def compute_measures_for_binary_segmentation(prediction, target):
    T = target.sum()
    P = prediction.sum()
    In = numpy.logical_and(prediction == 1, target == 1).sum()
...
def compute_iou_from_logits(preds, labels, num_labels):
    ...
    :param preds: 4D tensor of shape [batch_size, height, width, num_classes]
    ...
    with tf.variable_scope("IoU"):
        ...
        for label in range(num_labels):
            ...
            # intersection
            In = tf.logical_and(P, L)
            In = tf.reduce_sum(tf.cast(In, tf.float32))

            IOU = tf.cast(In, tf.float32) / tf.cast(U, tf.float32)
            # U might be 0!
            IOU = tf.where(tf.equal(U, 0), 1, IOU)
            IOU = tf.Print(IOU, [IOU], "iou" + str(label))
            IoUs.append(IOU)
...
def calc_iou(measures, n_imgs, ignore_classes):
    ...
    In = (numpy.diagonal(conf_matrix, axis1=1, axis2=2)).astype("float32")
    sum_predictions = numpy.sum(conf_matrix, axis=1)
    sum_labels = numpy.sum(conf_matrix, axis=2)
    U = sum_predictions + sum_labels - In
    ...
    for i, u in zip(In, U):
        ...
        if mask.any():
            ...
            logger.debug("warning, mask only consists of ignore_classes")
            iou = 1.0
        ious.append(iou)
