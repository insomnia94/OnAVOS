...
class DavisOneshotDataset(OneshotImageDataset):
    ...
    def _load_lucid_data_for_seq(self, video_idx):
        ...
        tensors = [create_tensor_dict(unnormalized_img=im, label=mask, tag=tag) for im, mask, tag in
                   zip(imgs, masks, tags)]
        return tensors

    def set_video_idx(self, video_idx):
        if self.use_lucid_data and self._video_idx != video_idx:
            if (DavisOneshotDataset._lucid_data_cache is not None) and \
               (DavisOneshotDataset._lucid_data_cache[0] == video_idx):
    ...
    def load_videos(self, fn, data_dir, video_range=None):
        ...
        elapsed = end - start
        print("loaded davis in", elapsed, "seconds")
        return DavisOneshotDataset._video_data
