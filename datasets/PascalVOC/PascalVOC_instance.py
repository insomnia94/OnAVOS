...
def postproc_labels_for_objectness(label):
    ...
    def my_postproc(ll):
        l_out = np.zeros_like(ll)
        l_out[np.logical_and(ll != 0, ll != 255)] = 1
        # do we want void here?
        l_out[ll == 255] = VOID_LABEL
        return l_out
...
class PascalVOCInstanceDataset(PascalVOCDataset):
    ...
    def create_label(self, img_path, label_path):
        ...
        if len(np.where(label == 1)[0]) < 20:
            print("Image with a small mask: " + img_path + " instance: " + repr(inst))
    ...
    def read_inputfile_lists(self):
        ...
        print("Time taken to read inputs: " + repr(end - start))
        return imgs, ans
