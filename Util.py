...
def load_wider_or_deeper_mxnet_model(model_path, session):
    ...
    var_dict = {v.name[:-2]: v for v in variables if "Adam" not in v.name and "_power" not in v.name
    ...
    for k, v in mxnet_dict.items():
        ...
        if var.get_shape() == val.shape:
            ...
            logger.info("warning, sizes for", k, "do not match, initializing matching part assuming",
            ...
            logger.info("skipping", k, "since the shapes do not match:", var.get_shape(), "and", val.shape)
